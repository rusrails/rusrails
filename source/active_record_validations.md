...
Чтобы проверить, какая валидация упала на невалидном атрибуте, можно использовать `errors.details[:attribute]`. Он возвращает массив хэшей с ключом `:error`, чтобы получить символ валидатора:
...
Также он может получать опцию `:accept`, которая определяет допустимые значения, которые должно считаться принятым. По умолчанию это "1", но его можно изменить.
...
Если хотите создать ограничение на уровне базы данных, чтобы предотвратить возможные нарушения валидации уникальности с помощью опции `:scope`, вы должны создать индекс уникальности на обоих столбцах вашей базы данных. Подробнее об индексах для нескольких столбцов смотрите [the MySQL manual](http://dev.mysql.com/doc/refman/5.7/en/multiple-column-indexes.html), или примеры ограничений уникальности, относящихся к группе столбцов в [the PostgreSQL manual](https://postgrespro.ru/docs/postgrespro/9.6/ddl-constraints).
...
Значение `String` в `:message` может опционально содержать любые из `%{value}`, `%{attribute}` и `%{model}`, которые будут динамически заменены, когда валидация провалится. Эта замена выполняется, если используется гем I18n, и место для вставки должно полностью совпадать, пробелы не допускаются.
...
В дополнение к методам `valid?` и `invalid?`, рассмотренные ранее, Rails предоставляет ряд методов для работы с коллекцией `errors` и проверки валидности объектов.
...
Возвращает экземпляр класса `ActiveModel::Errors`, содержащий все ошибки. Каждый ключ это имя атрибута, а значение это массив строк со всеми ошибками.
...
Можно указать тип валидатора в возвращаемом хэше подробностей об ошибке с помощью метода `errors.add`.
...
Чтобы улучшить подробности об ошибке, добавив, к примеру, недозволенные символы, можно передать дополнительные ключи в `errors.add`.
...
Все встроенные в Rails валидаторы заполняют хэш подробностей соответствующим типом валидатора.
...
Более того, при использовании хелперов форм Rails для создания форм, когда у поля происходит ошибка валидации, создается дополнительный `<div>` вокруг содержимого.
