...
NOTE: Поскольку процессы не деляться данными кэша при использовании `:memory_store`, то невозможно вручную считывать, записывать или очищать кэш через консоль Rails.
...
Rails по умолчанию генерирует слабые ETag. Слабые ETag позволяют семантически эквивалентным откликам иметь одинаковые ETag, даже если их тела не имеют точного совпадения. Это полезно, когда мы не хотим, чтобы страница заново генерировалась для небольших изменений в теле отклика.
