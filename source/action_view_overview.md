...
Опцию `object` можно использовать, чтобы непостредственно указать, какой объект рендирится в партиале; полезно, когда объект шаблона находится где-то еще (например, в другой переменной экземпляра или в локальной переменной).
...
При построении ответа, контроллер должен решить, где располагаются различные вьюхи. По умолчанию он смотрит только в папке `app/views`.
...
Action View будет сначала смотреть в этой директории при поиске вьюх.
...
Это добавить путь `app/views/direct` в конец поиска пути.
...
Нижеследующее является лишь кратким обзором хелперов, доступных в Action View. Рекомендуется обратиться к [документации API](http://api.rubyonrails.org/classes/ActionView/Helpers.html), покрывающей все хелперы более подробнее, но это является хорошей отправной точкой.
