...
Шаблоны Action View пишутся с помощью тегов встроенного Ruby, смешанных с HTML. Чтобы избежать загромождения вью шаблонным кодом, общее поведение для форм, дат и строк представлено рядом классов хелпера. В существующее приложение также легко добавлять новые хелперы.
...
Подробнее о хелперах можно узнать в [руководстве по хелперам Action View](/action-view-helpers) и [руководстве по хелперам форм Action View](/rails-form-helpers).
