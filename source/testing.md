...
```bash
$ rails test test/models/article_test.rb:6 # запускает определенный тест и строчку
```

Также можно запустить целую директорию тестов, предоставив путь к этой директории.

```bash
$ rails test test/controllers # запускает все тесты из определенной директории
```
...
NOTE: По умолчанию, выполнение `rails test` не будет запускать ваши системные тесты. Убедитесь, что вы выполняете `rails test:system`, чтобы на самом деле запустить их.
...
NOTE: Если выполнены шаги в разделе про базовую аутентификацию, то необходимо добавить авторизацию в заголовок каждого запроса, чтобы все тесты проходили:
...
Если ваши хелперы загромоздили `test_helper.rb`, их можно извлечь в отдельные файлы. Одним из хороших мест их хранения является `lib/test`.
...
### Проверка аргументов времени в заданиях

При сериализации аргументов задания, `Time`, `DateTime` и `ActiveSupport::TimeWithZone` теряют точность в микросекундах. Это означает, что сравнение десериализованного времени с фактическим не всегда работает. Чтобы компенсировать потерю точности, `assert_enqueued_with` и `assert_performed_with` уберут микросекунды из объектов времени в проверках аргументов.
...
### Пользовательские утверждения и тестирование трансляций внутри других компонент
