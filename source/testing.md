...
Любой метод, определенный в классе, унаследованном от `MiniTest::Unit::TestCase` (который является суперклассом для `ActiveSupport::TestCase`), начинающийся с `test` (чувствительно к регистру), просто вызывает тест. Таким образом, `test_password`, `test_valid_password` и `testValidPassword` это правильные имена тестов, и запустятся автоматически при запуске тестового случая.
...
Чтобы протестировать хелперы нужно всего лишь проверить, что результат метода хелпера соответствует тому, что вы ожидаете. Тесты, относящиеся к хелперам, расположены в директории `test/helpers`. Rails предоставляет генератор, с помощью которого генерируется хелпер и файл теста:
...
Сгенерированный файл теста содержит следующий код:
