...
* +to_prepare+: Запустится после того, как инициализаторы будут запущены для всех Railties (включая само приложение), но до нетерпеливой загрузки и построения стека промежуточного ПО. Что еще более важно, запустится после каждого запроса в +development+, но только раз (при загрузке) в +production+ и +test+.
...
*+initialize_cache+* Если +Rails.cache+ еще не установлен, инициализирует кэш, обращаясь к значению +config.cache_store+ и сохраняя результат как +Rails.cache+. Если этот объект отвечает на метод +middleware+, его промежуточное ПО вставляется до +Rack::Runtime+ в стеке промежуточного ПО.
...
*+load_config_initializers+* Загружает все файлы Ruby из +config/initializers+ в приложении, railties и движках. Файлы в этой директории могут использоваться для хранения конфигурационных настроек, которые нужно сделать после загрузки всех фреймворков.
