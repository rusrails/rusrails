...
[`config.middleware`]: /configuring#config-middleware
...
* Защищает от атак переназначения DNS, явно разрешая хосты, с которых может быть послан запрос. Смотрите инструкции в [руководстве по конфигурированию](/configuring#actiondispatch-hostauthorization).
...
[`config.session_store`]: /configuring#config-session-store
