...
Как известно, в Ruby имеется класс +UnboundMethod+, экземпляры которого являются методами, принадлежащими неопределенным методам без self. Метод +Module#instance_method+ возвращает несвязанный метод, например:
