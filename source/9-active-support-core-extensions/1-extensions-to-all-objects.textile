...
INFO: В Ruby 1.9 условие для строк использует учитывающий Unicode символьный класс +[:space:]+, поэтому, к примеру, U+2029 (разделитель параграфов) рассматривается как пробел. В Ruby 1.8 пробелом считается +\s+ вместе с идеографическим пробелом U+3000.
