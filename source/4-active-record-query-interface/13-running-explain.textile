...
Active Record способен запускать EXPLAIN автоматически на медленных запросов и логировать его результат. Эта особенность контролируется конфигурационным параметром

<ruby>
config.active_record.auto_explain_threshold_in_seconds
</ruby>

Если установить число, у любого запроса, превышающего столько секунд, будет автоматически включен и залогирован EXPLAIN. В случае с relations, порог сравнивается с общим временем, необходимым для извлечения записей. Таким образом, relation рассматривается как рабочая единица, вне зависимости от того, что применение нетерпеливой загрузки может вызвать несколько запросов за раз.
