...
[`form_with`](https://api.rubyonrails.org/classes/ActionView/Helpers/FormHelper.html#method-i-form_with) - это хелпер, помогающий писать формы. По умолчанию, `form_with` предполагает, что ваша форма будет использовать Ajax. Вы можете отказаться от этого поведения, передав `:local` опцию в `form_with`.
