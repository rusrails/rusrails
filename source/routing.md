...
Роутер Rails распознает URL и соединяет их с экшном контроллера или приложением Rack. Он также создает пути и URL, избегая необходимость жестко прописывать строки в ваших вьюхах.
...
Передавая `String` в `get` ожидается следующий формат - `controller#action`, при передаче `Symbol` произойдёт непосредственно прямое обращение к экшену, но вы также должны указать используемый `controller:`:
...
NOTE: Имеется исключения для ограничения `format`: так как это метод на объекте Request, это также неявный опциональный параметр для каждого пути. Ограничения сегмента имеют приоритет, и ограничение `format` применяется как есть, даже когда передано в хэше. Напрмиер, `get 'foo', constraints: { format: 'json' }` будет соответствовать `GET  /foo`, так как формат опциональный по умолчанию. Однако, [используя lambda](#advanced-constraints), как в `get 'foo', constraints: lambda { |req| req.format == :json }`, маршрут будет соответствовать только явным запросам JSON.
...
Пожалуйста, отметьте, что перенаправлением по умочанию является 301 "Moved Permanently". Учтите, что некоторые браузеры или прокси серверы закэшируют этот тип перенаправления, сделав старые страницы недоступными. Чтобы изменить статус отклика, можно использовать опцию `:status`:
...
Можно использовать опцию `:as` для задания префикса именованных маршрутных хелперов, создаваемых Rails для маршрута. Используйте эту опцию для предотвращения коллизий имен между маршрутами, использующими пространство путей. Например:
