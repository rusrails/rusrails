...
3. **Слишком много прекращенного кэша**
    ...
    При размещении статичных ассетов с каждым новым релизом кода, mtime (время последнего изменения) _всех_ этих файлов изменялось, принуждая всех удаленных клиентов получать их снова, даже если содержимое этих ассетов не менялось.
...
В прежних версиях Rails, все ассеты были расположены в субдиректориях `public`, таких как `images`, `javascripts` и `stylesheets`. Сейчас, с файлопроводом, предпочтительным местом расположения для этих ассетов стала директория `app/assets`. Файлы в этой директории отдаются промежуточной программой Sprockets.
...
WARNING: Если вы производите апгрейд с Rails 3, примите во внимание, что ассеты в `lib/assets` или `vendor/assets` доступны для включения в манифестах приложения, но больше не являются частью массива прекомпиляции. Что делать в этом случае, смотрите [Прекомпиляция ассетов](#precompiling-assets).
...
Апгрейд со старых версий Rails
------------------------------

Имеется несколько проблем при апгрейде c Rails 3.0 или Rails 2.x. Первая — это перемещение файлов из `public/` в новые места расположения. Смотрите раздел [Организация ассетов](#how-to-use-the-asset-pipeline), рассмотренный ранее, для указания правильных мест расположения файлов разных типов.

Далее выполняется обновление файлов различных сред с правильными дефолтными опциями.
