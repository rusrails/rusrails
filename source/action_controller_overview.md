...
Отметьте, что хэш `params` фактически является экземпляром `ActiveSupport::HashWithIndifferentAccess`, который ведет себя как хэш, который позволяет взаимозаменяемо использовать символы и строки как ключи.

### Параметры JSON

Если вы пишете приложение веб-сервиса, возможно вам более комфортно принимать параметры в формате JSON. Rails автоматически преобразует ваши параметры в хэш `params`, к которому можно получить доступ так же, как и к обычным данным формы.
...
class PeopleController < ActionController::Base
  ...
  # вызвано исключение ActionController::MissingParameter, которое будет
  ...
  private
    ...
    # использовать тот же самый списко разрешений при создании и обновлении
...
Представим ситуацию, когда вы хотите поместить в белый список атрибут, содержащий хэш с любыми ключами. Используя strong parameters нельзя разрешить эш с любыми ключами, но вы можете использовать простое присвоение для выполнения задачи:
