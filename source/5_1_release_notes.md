...
Приложения Rails можно интегрировать с [Webpack](https://webpack.js.org/), пакетированием ресурсов JavaScript, используя новый гем [Webpacker](https://github.com/rails/webpacker). Укажите флажок `--webpack` при генерации новых приложений, чтобы включить интеграцию с Webpack.

Это полностью совместимо с файлопроводом, который можно продолжать использовать для картинок, шрифтов, звуков и других ресурсов. Можно даже оставить некоторый код JavaScript, управляемый файлопроводом, а остальной код обрабатывать через Webpack. Все это управляется с помощью Yarn, который включен по умолчанию.
...
Rails 5.1 добавляет в DSL роутинга два новых метода, `resolve` в `direct`. Метод `resolve` позволяет настроить полиморфное соответствие моделей.
...
Метод `direct` позволяет создание хелперов для произвольного URL.
...
Когда тест порождает дополнительные треды, и эти треды получают соединения с базой данных, эти соединения теперь обрабатываются по-особенному:

Этим тредам достается единственное соединение, которое находится посреди управляемой транзакции. Это позволяет убедиться, что все треды видят базу данных в одном и том же состоянии, игнорируя внешнюю транзакцию. раньше такие дополнительные соединения были неспособны видеть, к примеру, строки фикстур.
...
Если ваши тесты порождают треды, и эти треды взаимодействуют, в то же время используя явные соединения с базой данных, это может вызвать взаимную блокировку (deadlock).

Самым простым способом отказаться от этого нового поведения является отключение транзакционных тестов на всех тестовых случаях, которые оно затрагивает.
...
*   Конфигурационный файл `config/secrets.yml` теперь загружается со всеми ключами как символами.
...
*   Устарел обработчик ERB Erubis в пользу Erubi.
...
*   Изменены `datetime_field` и `datetime_field_tag`, чтобы они генерировали поля `datetime-local`.
    ([Pull Request](https://github.com/rails/rails/pull/28061))
...
*   Удалена поддержка передачи аргументов и блока одновременно в `ActiveRecord::QueryMethods#select`.
...
*   Удален устаревший аргумент принудительной перезагрузки на методах чтения одиночной и множественной связи.
...
*   Удалено устаревшее поведение `#tables` и `#table_exists?` возвращать таблицы и представления, на возвращать только таблицы, но не представления.
...
*   Удалена устаревшая поддержка запросов с использованием запятых на LIMIT.
...
*   Устарел флажок `error_on_ignored_order_or_limit` в пользу `error_on_ignored_order`.
    ([Commit](https://github.com/rails/rails/commit/451437c6f57e66cc7586ec966e530493927098c7))

*   Устарел `sanitize_conditions` в пользу `sanitize_sql`.
    ([Pull Request](https://github.com/rails/rails/pull/25999))

*   Устарел `supports_migrations?` на адаптерах соединения.
...
*   Наследование непосредственно от `ActiveRecord::Migration` вызывает ошибку. Укажите версию Rails, для которой была написана миграция.
...
*   Устарела передача строк в опции условий `:if` и `:unless` на `set_callback` и `skip_callback`.
    ([Commit](https://github.com/rails/rails/commit/0952552)

### Значимые изменения

*   Починен парсинг продолжительности и перемещения во времени, он сделан более последовательным при смене DST.
