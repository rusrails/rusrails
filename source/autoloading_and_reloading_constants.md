...
*Вложенность* в любое заданное место — это коллекция из охваченных вложенных объектов класса и модуля для доступа снаружи. Вложенность в любом заданном месте можно просмотреть с помощью `Module.nesting`. Например, в предыдущем примере вложенностью (1) является
...
У назначения констант есть специальное правило, благодаря которому это происходит: если назначаемый объект является анонимным классом или модулем, Ruby устанавлевает имя объекта по константе.
...
`config.autoload_paths` доступна и в конфигурационных файлов, относящихся к окружению, но любые изменения, сделанные вне `config/application.rb` будут безрезультатны.
...
Наследование с единой таблицей (STI) — это особенность Active Record, позволяющая храненить иерархию моделей в одной отдельной таблице. API таких моделей знает об иерархии и инкапсулирует некоторые общие потребности. Например, имеем следующие классы:
