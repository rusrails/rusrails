...
Если необходимо [принудительно использовать ссылочную целостность на уровне базы данных](/active-record-migrations#foreign-keys), добавьте опцию `foreign_key: true` в вышеприведенное объявление 'reference' столбца.
...
Если колбэк `before_add` вызывает `:abort`, объект не будет добавлен в коллекцию. Подобным образом, если колбэк `before_remove` вызывает `:abort`, объект не убирается из коллекции:
