...
Если в вашем приложении есть STI, посмотрите на соответствующий раздел в руководстве [Автозагрузка и перезагрузка констант (режим Zeitwerk)](/constant_autoloading_and_reloading#single-table-inheritance).
...
Существующие приложения могут включить это новое поведение установив `config.active_storage.replace_on_assign_to_many` в `true`. Старое поведение будет устаревшим в Rails 6.1 и убрано в последующем релизе.
...
Если ваши вьюхи используют `render :text`, они перестанут работать. Новый способ рендерить вьюхи с типом MIME `text/plain` — использовать `render :plain`.
