...
Затем убедитесь, что все классы задач наследуются от него.
...
Нетерпеливая загружка приложения - это часть процесса загрузки, поэтому константы верхнего уровня все еще автоматически загружаются, их файлы не нужно требовать.
...
Чтобы запустить тесты, просто напишити `bin/rails test`.
...
Если вы не хотите, чтобы ваш файл обрабатывался с помощью обработчика raw, следует добавить расширение к своему файлу, чтобы он парсился подходящим обработчиком шаблона.
