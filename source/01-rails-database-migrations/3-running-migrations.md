...
Это вернёт ситуацию к последней миграции, так же при обращении к методу `change` или `down`. Если нужно отменить несколько миграций, можно указать параметр `STEP`:
