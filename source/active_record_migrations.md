...
Миграционный метод `create_join_table` создает соединительную таблицу HABTM. Обычное использование будет таким:
...
Метод `revert` так же может принимает блок. Это может быть полезным для отката выбранной части предыдущих миграций. Для примера, давайте представим что `ExampleMigration` закомичена, а позже мы решили, что было бы лучше использовать валидации Active Record, вместо ограничения `CHECK`, для проверки zipcode.
