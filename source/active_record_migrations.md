...
Это добавит новый внешний ключ к столбцу `author_id` таблицы `articles`. Ключ ссылается на стобец `id` таблицы `articles`. Если имена стобцов не могут быть произведены из имен таблиц, можно использовать опции `:column` и `:primary_key`.
...
NOTE: Active Record поддерживает внешние ключи только для отдельных столбцов. Чтобы использовать состаные внешние ключи, требуются `execute` и
`structure.sql`.
...
# уберем внешний ключ для определенного стобца
...
Метод `revert` так же может принимает блок. Это может быть полезным для отката выбранной части предыдущих миграций. Для примера, давайте представим что `ExampleMigration` закомичена, а мы позже решили, что было бы лучше использовать валидации Active Record, вместо ограничения `CHECK`, для проверки zipcode.
