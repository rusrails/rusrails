h1. Методы хелпера

Rails предоставляет различные методы хелпера в Active Record, Action Controller и Action View для измерения времени, затраченного на заданный кусок кода. Метод называется +benchmark()+ во всех трех компонентах.

h4. Модель

<ruby>
Project.benchmark("Creating project") do
  project = Project.create("name" => "stuff")
  project.create_manager("name" => "David")
  project.milestones << Milestone.all
end
</ruby>

Это произведет бенчмаркинг кода, заключенного в блок +Project.benchmark("Creating project") do...end+ и напечатает результат в файл лога:

<ruby>
Creating project (185.3ms)
</ruby>

Пожалуйста, обратитесь к "API docs":http://api.rubyonrails.org/classes/ActiveRecord/Base.html#M001336, чтобы узнать дополнительные опции для +benchmark()+

h4. Контроллер

Подобным образом можно использовать этот метод хелпера в "контроллерах":http://api.rubyonrails.org/classes/ActionController/Benchmarking/ClassMethods.html#M000715

<ruby>
def process_projects
  self.class.benchmark("Processing projects") do
    Project.process(params[:project_ids])
    Project.update_cached_projects
  end
end
</ruby>

NOTE: +benchmark+ это метод класса в контроллерах

h4. Вьюха

И во "вьюхах":http://api.rubyonrails.org/classes/ActionController/Benchmarking/ClassMethods.html#M000715:

<erb>
<% benchmark("Showing projects partial") do %>
  <%= render @projects %>
<% end %>
</erb>
