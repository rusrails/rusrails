...
* Язык [Ruby](https://www.ruby-lang.org/en/downloads) версии 1.9.3 или новее
...
TIP: Существует ряд инструментов для помощи в быстрой установке Ruby и Ruby on Rails в вашей системе. Пользователи Windows могут использовать [Rails Installer](http://railsinstaller.org), а пользователи Mac OS X могут использовать [Tokaido](https://github.com/tokaido/tokaidoapp).
...
Если у вас нет установленного Ruby обратитесь к [ruby-lang.org](https://www.ruby-lang.org/en/installation/) для поиска возможных способов установки Ruby на вашей платформе.

Многие популярные UNIX-подобные ОС поставляются с приемлемой версией SQLite3. Пользователи Windows и прочие могут обратиться к инструкциям по установке на [сайте SQLite3](https://www.sqlite.org). Проверьте, что он корректно установлен и содержится в вашем PATH:
...
Если выводится что-то вроде "Rails 4.2.0", можно продолжать.
...
[strong parameters](/action-controller-overview#strong-parameters), что требует от нас сказать Rails, какие именно параметры разрешены в экшнах нашего контроллера.
...
Добавьте соответстующий этому маршруту экшн `index` внутри `ArticlesController` в файле `app/controllers/articles_controller.rb`. Когда мы пишем экшн `index`, обычной практикой является разместить его как первый метод в контроллере. Сделаем это:
...
TIP: Чтобы генерировать Rails Guides локально с помощью рейк таска `doc:guides`, вам нужно установить RedCloth и Nokogiri гемы. Добавьте их в ваш `Gemfile` и запустите каманду `bundle install`, и можно продолжать.
