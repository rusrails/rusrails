...
TIP: Заметьте, что `def post_params` является приватным. Этот новый подход предотвращает изменение атрибутов модели злоумышленником, с помощью манипуляции с хешем, передаваемым в модель. За подробностями обратитесь к [Strong Parameters](http://weblog.rubyonrails.org/2012/3/21/strong-parameters/).
