...
В терминах реализации: Маршруты это правила, написанные на Ruby [DSL (предметно-ориентированном языке)](https://ru.wikipedia.org/wiki/Предметно-ориентированный_язык). Контроллеры это классы Ruby, и их публичные классы это экшны. И вью это шаблоны, обычно написанные на смеси HTML и Ruby.
...
Важно отметить, что мы только что *инициализировали* этот объект. Этот объект вообще не сохранен в базе данных. В настоящий момент он доступен только в консоли. Чтобы сохранить его в базу данных, нужно вызвать [`save`](https://api.rubyonrails.org/classes/ActiveRecord/Persistence.html#method-i-save):
