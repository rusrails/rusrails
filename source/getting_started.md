...
Когда мы делаем запрос к нашему приложению Rails, мы делаем запрос к определенному _маршруту_. Для начала, давайте создадим маршрут в `config/routes.rb`:
...
Это _маршрутный файл_ вашего приложения, который содержит записи на специальном [DSL (предметно-ориентированном языке)](https://ru.wikipedia.org/wiki/Предметно-ориентированный_язык), сообщающим Rails, как связать входящие запросы с контроллерами и экшнами.

Строчка, которую мы только что добавили, сообщает, что мы собираемся соотнести запрос `GET /articles` с `articles#index`. Эта строка передана как опция `to`, представляющая _контроллер_ и _экшн_, которые будут ответственны за обработку этого запроса.

Контроллеры — это классы, которые группируют общие методы для обработки определенного _ресурса_. Методы внутри контроллеров названы "экшнами", так как на английском они называются "action", и у этого слова нет прямого перевода, это слово производное от _act upon_.

Для создания нового контроллера, нужно запустить генератор "controller" и сказать ему, что вы хотите контроллер с именем "articles" с экшном по имени "index", вот так:
...
Наиболее важным из них является контроллер, расположенный в `app/controllers/articles_controller.rb`.

Давайте посмотрим на этот контроллер:
...
Откройте файл `app/views/articles/index.html.erb` в текстовом редакторе. Удалите весь существующий в файле код и замените его на следующую строчку кода:

```html
<h1>Hello, Rails!</h1>
```

Если переключитесь на браузер и сделать запрос к <http://localhost:3000/articles>, то увидите, что этот текст появился на странице.

### Настройка домашней страницы приложения

Теперь, когда мы сделали контроллер и вьюху, нужно сказать Rails, что мы хотим увидеть "Hello Rails!". В нашем случае мы хотим это увидеть, когда зайдем в корневой URL нашего сайта, <http://localhost:3000>. Однако сейчас это место заняла тестовая страница "Yay! You're on Rails!".
...
Эта ошибка произошла, поскольку маршрут нуждается в определенном контроллере, чтобы обслужить запрос. Решением этой проблемы является создание контроллера с именем `ArticlesController`. Это будет сделано запуском команды:
...
Контроллер - это класс, унаследованный от `ApplicationController`. В этом классе вы должны определить методы, которые станут экшнами для этого контроллера. Эти экшны будут выполнять операции CRUD со статьями в вашей системе.

NOTE: В `Ruby` методы бывают `public`, `private` и `protected`, но экшнами контроллеров могут быть только `public` методы. Подробнее смотрите в [Programming Ruby](https://www.ruby-doc.org/docs/ProgrammingRuby/).
...
Сообщение указывает, какой шаблон отсутствует. В нашем случае, шаблон `articles/new`. Rails сперва ищет этот шаблон. Если не находит, он пытается загрузить шаблон с именем `application/new`, поскольку `ArticlesController` унаследован от `ApplicationController`.

Затем сообщение содержит `request.formats`, указывающий формат шаблона, обслуживающего отклик. Он установлен в `text/html`, так как мы запросили эту страницу с помощью браузера, поэтому Rails ищет шаблон HTML.
...
Если теперь обновить страницу, вы увидите точно такую форму как в начале примера.
...
Если теперь еще раз отправить форму, на странице ничего не поменяется. Не беспокойтесь! Это так, потому что Rails по умолчанию возвращает отклик `204 No Content` для экшна, если мы не указали, какой должен быть отклик. Мы добавили экшн `create`, но не указали ничего о том, каким должен быть отклик. В нашем случае, экшн `create` должен сохранить нашу новую статью в базу данных.
...
TIP: За подробностями обратитесь к [этой статье по Strong Parameters](https://weblog.rubyonrails.org/2012/3/21/strong-parameters/).
...
TIP: В среде development (с которой вы работаете по умолчанию), Rails перезагружает ваше приложение с каждым запросом браузера, так что не нужно останавливать и перезапускать веб-сервер при внесении изменений.
...
NOTE:  Для рендера вьюхи мы используем `edit`. Для фактического сохранения изменений в Article позже мы добавим экшн `update`.
...
Также, передача объекта модели в `form_with`, наподобие `model: @article` во вьюхе редактирования выше, заставит хелпер формы заполнить поля формы соответствующими значениями объекта. Передача в пространство имен символа, такого как `scope: :article`, что было сделано во вьюхе new, только создаст пустые поля формы. Подробнее в [документации по form_with](https://api.rubyonrails.org/classes/ActionView/Helpers/FormHelper.html#method-i-form_with).
...
Все, за исключением объявления `form_with`, осталось тем же самым. Причина, по которой можно использовать это более короткое объявление `form_with` по сравнению с другим формами, в том, что `@article` — это *ресурс*, соответствующий полному набору ресурсных маршрутов, и Rails способен определить, какой использовать URI и method. Подробнее об этом использовании `form_with` смотрите в [Resource-oriented style](https://api.rubyonrails.org/classes/ActionView/Helpers/FormHelper.html#method-i-form_with-label-Resource-oriented+style).
...
Затем во вьюхе его можно вызвать как любой другой метод класса:
