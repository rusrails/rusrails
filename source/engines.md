...
Эта изоляция в пространство имен означает, что модель, созданная с помощью `rails g model`, например `rails g model article`, не будет называться `Article`, а будет помещена в пространство имен и названа `Blorgh::Article`. Кроме того, таблица для модели будет помещена в пространство имен, и станет `blorgh_articles`, а не просто `articles`. Подобно пространству имен моделей, контроллер с именем `ArticlesController` будет `Blorgh::ArticlesController`, и вьюхи для этого контроллера будут не в `app/views/articles`, а в `app/views/blorgh/articles`. Рассыльщики также помещены в пространство имен.
...
В директории `app/helpers` есть директория `blorgh`, содержащая файл `application_helper.rb`. Этот файл предоставит любой общий функционал для хелперов engine. Директория `blorgh` является местом для других хелперов для engine. Помещая их в директорию пространства имен, вы предотвращаете возможные пересечения с идентично названными хелперами других engine или даже приложения.

В директории `app/jobs` есть директория `blorgh`, содержащая файл `application_job.rb`. Этот файл предоставит любой общий функционал для заданий engine. Директория `blorgh` является местом для других заданий для engine. Помещая их в директорию пространства имен, вы предотвращаете возможные пересечения с идентично названными заданиями других engine или даже приложения.

В директории `app/mailers` есть директория `blorgh`, содержащая файл `application_mailer.rb`. Этот файл предоставит любой общий функционал для рассыльщиков engine. Директория `blorgh` является местом для других рассыльщиков для engine. Помещая их в директорию пространства имен, вы предотвращаете возможные пересечения с идентично названными рассыльщиками других engine или даже приложения.
...
Для запуска этих миграций в контексте приложения просто выполните `rails db:migrate`. При входе в engine по адресу `http://localhost:3000/blog`, статей не будет, поскольку таблица, созданная в приложении, отличается от той, что была создана в engine. Сходите, поиграйте с только что смонтированным engine. Он точно такой же, как когда он был только engine.
...
Далее должна быть запущена команда `rails db:migrate`, чтобы для дальнейшего использовании в приложении создалась таблица `users`.
...
#### Заметка о переопределении и загрузке кода

Поскольку на эти переопределения не ссылается само приложение Rails, система автозагрузки Rails не сработает и не загрузит ваши переопределения. Это означает, что их необходимо затребовать самостоятельно.
...
Это применимо не только к переопределениям, а вообще ко всему, что добавляется в engine, и на что не ссылается основное приложение.

#### Изменение существующих классов с использованием Class#class_eval
...
Это сообщит Sprockets добавить ресурсы engine при вызове `rails assets:precompile`.
