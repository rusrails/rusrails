...
Вы можете вызвать `instance_variables` для вывода всех инстанс переменных, доступных в контексте. Если вы хотите получить список всех локальных переменных, вы моете сделать это с помощью `local_variables`.
