...
Например, предположим, что в приложении имеется модель `User`. Если необходимо, чтобы у каждого пользователя был аватар, запустите команду генерации модели так:

```ruby
bin/rails generate model User avatar:attachment
```

или определите модель `User` следующим образом:
...
Например, предположим, что в приложении имеется модель `Message`. Если необходимо, чтобы у каждого сообщения было много изображений, запустите генератор модели так:

```ruby
bin/rails generate model Message images:attachments
```

или определите модель `Message` следующим образом:
