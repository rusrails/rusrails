...
| `active_storage_attachments`     | Полиморфная соединительная таблица, [соединяющая ваши модели с blobs](#attaching-files-to-records). Если имя класса вашей модели изменится, вам потребуется выполнить миграцию для этой таблицы, чтобы обновить `record_type` на новое имя класса вашей модели. |
| `active_storage_variant_records` | Если включено [отслеживание вариантов](#attaching-files-to-records), хранит записи для каждого сгенерированного варианта. |

WARNING: Если используются UUID вместо чисел в качестве первичного ключа моделей, необходимо установить `Rails.application.config.generators { |g| g.orm :active_record, primary_key_type: :uuid }` в файле конфигурации.
...
Добавьте гем [`google-cloud-storage`](https://github.com/GoogleCloudPlatform/google-cloud-ruby/tree/main/google-cloud-storage) в `Gemfile`:
...
(attaching-files-to-records) Прикрепление файлов к записям
----------------------------------------------------------
...
NOTE: Поскольку Active Storage полагается на полиморфные связи, а [полиморфные связи](/active-record-associations#polymorphic-associations) полагаются на хранение имен классов в базе данных, эти данные должны оставаться синхронизированы с именем класса, используемым в Ruby-коде. При переименовании классов, использующих `has_one_attached`, обязательно также обновите имена классов в столбце полиморфного типа `active_storage_attachments.record_type` соответствующих строк.
...
### Замена и добавление вложений

По умолчанию в Rails при прикреплении файлов к связи `has_many_attached` любые существующие вложения будут заменены.

Чтобы сохранить существующие вложения, можно использовать скрытые поля формы с [`signed_id`][ActiveStorage::Blob#signed_id] каждого вложенного файла:
...
Вложения не отправляются в сервис хранения до успешного `save` связанной записи. Это означает, что если отправка формы не пройдет валидацию, все новые вложения будут потеряны и их потребуется загрузить снова. Однако, поскольку [прямые загрузки](#direct-uploads) сохраняются до отправки формы, их можно использовать для сохранения загрузок при ошибке валидации:
...
1. Обработает файл и загрузит обработанный файл, если необходимо.
2. Вернет редирект с кодом `302` на файл, направляя либо:
  * на удаленный сервис (например, S3).
  * или на `ActiveStorage::Blobs::ProxyController`, который вернет содержимое файла, если включен [режим прокси](#proxy-mode).

Ленивая загрузка файла позволяет использовать такие функции, как [одноразовые URL](#public-access), не замедляя начальную загрузку страницы.
...
### Пользовательские решения для перетаскивания элементов
