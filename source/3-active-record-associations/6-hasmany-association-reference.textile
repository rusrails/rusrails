...
NOTE: Если определите +:finder_sql+, но не +:counter_sql+, тогда счетчик SQL будет автоматически создан, заменив выражение +SELECT ... FROM+ из выражения +:finder_sql+ на +SELECT COUNT(*) FROM+.
...
Если установить опцию +:dependent+ как +:restrict+, то удаление объекта будет прервано, если сущестует зависимый связанный объект, и будет вызвано исключение +DeleteRestrictionError+.

NOTE: Поведением по умолчанию для +:dependent => :restrict+ является вызов +DeleteRestrictionError+, когда существуют связанные объекты. Начиная с Rails 4.0, это поведение устарело в пользу добавления ошибки в основную модель. Чтобы заглушить это предупреждение в Rails 4.0, следует исправить ваш код не ожидать это исключение, и добавить +config.active_record.dependent_restrict_raises = false+ в конфигурацию приложения. (Rails4)
