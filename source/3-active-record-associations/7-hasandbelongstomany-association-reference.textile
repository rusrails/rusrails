...
Опция +:select+ позволяет переопределить SQL предложение +SELECT+, которое используется для получения данных о связанном объекте. По умолчанию Rails получает все столбцы.
