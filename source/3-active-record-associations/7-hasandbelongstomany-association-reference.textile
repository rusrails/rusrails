...
Опция +:select+ позволяет переопределить SQL преложение +SELECT+, которое используется для получения данных о связанном объекте. По умолчанию Rails получает все столбцы.
