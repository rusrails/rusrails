...
Как упомянуто в [Rails README](https://github.com/rails/rails/blob/main/README.md), от каждого, участвующего в коде Rails и его подпроектах, системах отслеживания ошибок, чатах и списках рассылок, ожидается следование [нормам поведения](https://rubyonrails.org/conduct/) Rails.
...
Ваш отчет о проблеме должен, как минимум, содержать заголовок и ясное описание проблемы. Следует включить как можно больше релевантной информации и пример кода, демонстрирующего проблему. Будет даже лучше, если вы включите юнит-тест, тестирующий ожидаемое поведение. Вашей целью должно быть упрощение для себя — и для других — в воспроизведении программной ошибки и понимании, как ее починить.

А затем, не теряйте надежды! Если у вас некритичная программная ошибка, не ожидайте, что этот отчет о проблеме автоматически увидит какую-либо активность, или что другие переключатся на нее, чтобы починить. Вы создаете этот отчет о проблеме, чтобы другие с той же проблемой смогли подтвердить ошибку и взаимодействовать с вами, чтобы ее решить.
...
Кроме сообщения о проблемах, вы можете помочь основной команде разрешить существующие проблемы, предоставив обратную связь о них. Если вы новичок в разработке ядра Rails, это отличный способ сделать первые шаги, познакомиться с кодовой базой и процессами.
...
NOTE: Для изменений документации заголовок вашего пул-реквеста должен содержать [ci skip]. Это позволит опустить запуск тестового набора, помогая сократить расходы на сервер. Помните, что следует пропускать CI только когда ваши изменения затрагивают исключительно документацию.
...
TIP: Косметические изменения, не добавляющие ничего ощутимого в стабильность, функциональность или тестируемость Rails, обычно не принимаются (читайте подробнее об [обоснованиях этого решения](https://github.com/rails/rails/pull/13771#issuecomment-32746700)).
...
Даже если вы думаете, что содержание комита очевидно, оно может быть
...
Очень вероятно, что были сделаны другие изменения в main, пока вы работали. Получите их:
...
Если прошло больше недели, и никто не ответил, можно попытаться немного ускорить процесс. Для этого используйте [рассылку rubyonrails-core](https://discuss.rubyonrails.org/c/rubyonrails-core). Также можно оставить комментарий в своем пул-реквесте.
...
Отметьте, что пул-реквест может быть помечен как "Approved" кем-то, у кого нет доступа для его слияния. Могут быть затребованы последующие изменения перед тем, как члены основной команды или команды отправляющего примут его. Чтобы предотвратить путаницу, при обратной связи на чей-то пул-реквест избегайте пометку его как "Approved", пожалуйста.
...
NOTE: Перед работой над старшими версиями, проверьте [политику поддержки](/maintenance-policy).

#### Бэкпортирование

Изменения, слитые в main, предназначены для следующего главного релиза Rails. Иногда полезно, распространить ваши изменения на поддерживаемые релизы для старших стабильных веток. Как правило, хорошими кандидатами для бэкпортирования являются исправления безопасности и исправления программных ошибок, в то время как новые особенности и исправления, изменяющие поведение, не будут приняты. Когда сомневаетесь, лучше всего проконсультироваться с членом основной команды Rails до бэкпортирования ваших изменений, чтобы избежать потери времени.

Для простых изменений, легчайшим способом бэкпортировать ваши изменения является [извлечь diff ваших изменений в main и применить его в целевой ветке](https://ariejan.net/2009/10/26/how-to-create-and-apply-a-patch-with-git).

Сначала убедитесь, что только ваши изменения составляют разницу между вашей текущей веткой и main:
...
Это хорошо работает для простых изменений. Однако, если изменения сложные, или если код в main сильно отличается от целевой ветки, это потребует большей работы с вашей стороны. Сложность бэкпортирования отличается от случая к случаю, и даже иногда не стоит усилий.

Как только вы разрешили все конфликты и убедились, что проходят все тесты, отправьте свои изменения и откройте отдельный пул-реквест для своего порта. Также важно отметить, что старшие ветки могут иметь другой список целевых версий, чем в main. По возможности, лучше до от отправки пул-реквеста протестировать бэкпорт локально на старшей версии Ruby, разрешенной `rails.gemspec` целевой ветки.
