...
*   Когда сервер Rails настроен обслуживать статичные ассеты, gzip ассеты также будут обслужены, если клиент их поддерживает и предварительно генерирует файл gzip (`.gz`) на диск. По умолчанию asset pipeline генерирует файлы `.gz` для всех сжимаемых ассетов. Обслуживание gzip файлов минимизирует передаваемые данные и ускоряет запрос к ассету. Всегда [используйте CDN](https://github.com/rusrails/rusrails/blob/4.2/source/asset_pipeline.md#cdns-cdn), если обслуживаете файлы ассетов на сервере Rails в production.
...
*   Новое [руководство](https://github.com/rusrails/rusrails/blob/6.1/source/autoloading_and_reloading_constants_classic_mode.md) про автозагрузку и перезагрузку констант.
