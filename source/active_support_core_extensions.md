...
class User < ActiveRecord::Base
  # Теперь можно образаться к столбцу email как "login".
...
Метод `to_formatted_s` представляет спецификатор по умолчанию "F". Это означает, что простой вызов `to_formatted_s` или `to_s` выведет представление с плавающей запятой вместо инженержной нотации:
...
В случае коллизиии будет выбрано одно из значений. Выбранное значение не всегда будет одним и тем же для одного и того же хэша:
...
#### `transform_values` && `transform_values!` (Rails 4.2)
...
# => {nil=>"", 1=>"1", :x=>"A"}
```
Твуже имеется восклицательный вариант `transform_values!`, который применяест операции блока к значениям самого получателя.

NOTE: Определено в `active_support/core_text/hash/transform_values.rb`.
...
Active Support определяет `Time.current` как сегодняшний день в текущей временной зоне. Он похож на `Time.now`, за исключением того, что он учитывает временную зону пользователя, если она определена. Он также определяет условия экзеппляра `past?`, `today?` и `future?`, все они относительны к `Time.current`.
