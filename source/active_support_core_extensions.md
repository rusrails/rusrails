...
Для удобства `class_attribute` определяет также предикат экземпляра, являющееся двойным отрицанием того, что возвращает ридер экземпляра. В вышеописанном примере оно может вызываться `x?`.
...
Предикат `exclude?` тестирует, является ли заданный объект **не** принадлежащим коллекции. Это противоположность встроенного `include?`:
...
Бэкпорт имеет тот же интерфейс, где отсутствуют побочные эффекты в вызывающем методе, например, не устанавливает `$1` и ему подобное, но он не имеет преимуществ в скорости. Его цель состоит в том, чтобы предоставить возможность писать код, совместимый с версией 2.4. Rails сам использует этот предикат внутри, например.
