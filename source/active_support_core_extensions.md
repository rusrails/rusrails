...
Метод `truncate_bytes` возращает копию получателя, обрезанную к максимуму `bytesize` байт:
...
У `excluding` есть псевдоним `without`.
...
Метод `including` возвращает новый массив, включающий переданные элементы:
...
Методд `excluding` возвращает копию Arrayс исключенными указанными элементами. Это оптимизация `Enumerable#excluding`, использующая `Array#-` вместо `Array#reject` по причинам производительности.
...
Также имеется восклицательный вариант `deep_transform_values!`, деструктивно конвертирующий все значения с помощью операции блока.
...
В Ruby 1.9 `prev_year` и `next_year` возвращают время в том же дне/месяце в предыдущем или следующем году:
