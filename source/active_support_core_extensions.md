...
Хэши также отвечают на `to_query`, но в другом ключе. Если аргументы не заданы, вызов генерирует сортированную серию назначений ключ/значение, вызвав `to_query(key)` на его значениях. Затем он соединяет результат с помощью "&":
...
Предыдущий пример генерирует `address_street`, а не `street`.
...
Генерация райтер-метода экземпляра может быть отключено установлением опции `:instance_writer` в false, как в
...
Генерация ридер-метода может быть отключено установлением опции `:instance_reader` в `false`.
...
Макросы `cattr_reader`, `cattr_writer` и `cattr_accessor` являются аналогами их коллег `attr_*`, но для классов. Они инициализируют переменную класса как `nil`, если она уже существует, и генерируют соответствующие методы класса для доступа к ней:
...
Как показывает предыдущий пример, Active Support знает некоторые неправильные множественные числа и неисчислимые существительные. Встроенные правила могут быть расширены в `config/initializers/inflections.rb`. Этот файл генерируется командой `rails` и имеет инструкции в комментариях.
...
Следующий пункт особенно заметен для некоторых перечислений:
...
Как изображено в примере, формат `:db` генерирует SQL условие `BETWEEN`. Это используется Active Record в его поддержке интервальных значений в условиях.
