...
Active Support представляет лучшую реализацию `to_json`, чем гем `json`, обычно представленный для объектов Ruby. Это так, потому что некоторые классы, такие как `Hash` и `OrderedHash`, нуждаются в специальной обработке для подходящего представления в JSON.

Active Support также представляет реализацию `as_json` для класса `Process::Status`.

NOTE: Определено в `active_support/core_ext/object/to_json.rb`.
...
NOTE: Определено в `actionpack/lib/abstract_controller/helpers.rb`.
