...
Теперь, когда вы узнали, как генераторы используются _внутри_ приложения, знаете ли вы, что они используются и для _генерации_ приложения тоже? Этот тип генераторов называют "template". Далее идет краткий обзор Templates API. Подробную информацию смотрите в [руководстве по шаблонам приложения Rails](/rails-application-templates).
...
* `:group` - Группа в `Gemfile`, где дожен быть гем.
